...
#ВОПРОС есть ee -> e, есть e -> e тогда этот способ не сработает и надо всё равно учитывать порядок правил
...
def get_grafems(word):
    ...
    #return list(word)
    # со вторым словом надо как то понять что th - это отдельная графеми хз как сделать
...
# 2) Проблема в том, что для каждого правила придеться писать свою функцию (сложная функция)
